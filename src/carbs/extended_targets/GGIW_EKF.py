...
class GGIW_ExtendedKalmanFilter(ExtendedKalmanFilter):
    ...
    def predict(
        ...
        if self.__model is not None:
            ...
            if self.cont_cov:
                ...
                cur_cov = tmp.reshape(cur_cov.shape)
            else:
                cur_cov = state_mat @ cur_cov @ state_mat.T + self.proc_noise

            next_cov = cur_cov.copy()
            ...
            next_IWdof = 2 * GGIW_obj + 2 + np.exp(-timestep / self.tau) * (cur_IWdof - 2 * GGIW_obj.d - 2)
            next_IWshape = cur_IWshape

            next_dist = GGIW(alpa=next_alpha, beta=next_beta, mean=next_state, covariance=next_cov, IWdof=next_IWdof, IWshape=next_IWshape)
    ...
    def correct(
        ...
        if self.__model is not None:
            ...
            kalman_gain = cur_cov @ meas_mat @ inv_inov_cov    # Kalman gain finally

            X_hat_power =  X_hat**(0.5)

            inov_cov_power = inov_cov**(-0.5)
            ...
            meas_fit_prob = self._calc_meas_fit(meas,GGIW_obj,next_dist,X_hat,inov_cov) # meas, est_meas, inov_cov) # NEED TO UPDATE _calc_meas_fit 
    ...
    def _calc_meas_fit(self, meas, GGIW_pred, GGIW_upd, X_hat, inov_cov):
        ...
        L = (np.pi()**W * W) ** (-d/2) * la.det(pred_IWshape)**((pred_IWdof-d-1)/2) / \
                la.det(upd_IWshape)**((upd_IWdof-d-1)/2) * stats.special.multigammaln((upd_IWdof-d-1)/2,d) / \
                stats.special.multigammaln((pred_IWdof-d-1)/2,d) * la.det(X_hat)**0.5 / la.det(inov_cov)**0.5 * \
                stats.special.gamma(upd_alpha)*pred_beta**(pred_alpha) / (stats.special.gamma(pred_alpha)*upd_beta**(upd_alpha))
